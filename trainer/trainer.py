...
class Trainer:
    ...
    def __init__(self, model, optimizer, config, data_loader,
        ...
        self.m_metrics = MetricTracker('mEF', 'mEP', 'mER', 'mEA',
                                       writer=self.writer if self.local_master else None)
    ...
    def _train_epoch(self, epoch):
        ...
        for step_idx, input_data_item in enumerate(self.data_loader):
            ...
            if self.config['trainer']['anomaly_detection']:
                ...
                self.optimizer.zero_grad()
                # model forward
                # self.logger_info('Image Name: ' + input_data_item['filenames'][0])
                output = self.model(**input_data_item)
                # calculate loss
                crf_loss = output['crf_loss']
                # total_loss = torch.sum(crf_loss) + self.gl_loss_lambda * torch.sum(gl_loss)
            ...
            if step_idx % self.log_step == 0:
                self.logger_info('Train Epoch:[{}/{}] Step:[{}/{}] Total Loss: {:.6f} CRF_Loss: {:.6f}'.
                                 format(epoch, self.epochs, step_idx, self.len_step,
                                        avg_loss.item(), avg_crf_loss.item()))
            ...
            if self.do_validation and step_idx % self.val_step_interval == 0:
                ...
                self.m_metrics.writer.set_step((epoch - 1) * self.len_step + step_idx - 1) if self.local_master else None
                self.m_metrics.update_metric('mEF', {
                    'overall': val_result_dict['overall']['mEF']
                })
                self.m_metrics.update_metric('mEP', {
                    'overall': val_result_dict['overall']['mEP']
                })
                self.m_metrics.update_metric('mER', {
                    'overall': val_result_dict['overall']['mER']
                })
                self.m_metrics.update_metric('mEA', {
                    'overall': val_result_dict['overall']['mEA']
                })
                if best:

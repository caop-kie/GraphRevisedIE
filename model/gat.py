...
class GNNBaseLayer(torch.nn.Module):
    """
    ...
    def __init__(self, num_in_features, num_out_features, num_of_heads, concat=True, activation=nn.ELU(),
                 dropout_prob=0.6, add_skip_connection=True, bias=True, log_attention_weights=False, batch_size=2):
...
class GATNet(torch.nn.Module):
    ...
    def __init__(self, num_of_layers, num_heads_per_layer, num_features_per_layer, add_skip_connection=True, bias=True,
        ...
        for i in range(num_of_layers):
            layer = GATLayer(
                num_in_features=num_features_per_layer[i],  # consequence of concatenation
    ...
    def forward(self, data, boxes_num):
        for i, gat_layer in enumerate(self.gat_net):
            data, boxes_num = gat_layer(data, boxes_num)
        return data


class GATLayer(GNNBaseLayer):
    """
    ...
    def forward(self, x, boxes_num):
        #
        # Step 1: Linear Projection + regularization
        # B, N, D = x.shape
        B, N, D = x.shape
        NH = self.num_of_heads
        ...
        # B, N, NH, D
        # each head will have the same input feature
        nodes_features_proj = x.unsqueeze(2).expand(B, N, NH, D)
        ...
        for i in range(B):
            num = int(boxes_num[i])
            src_index = torch.arange(num, device=x.device).unsqueeze(-1).expand(1, num, num).reshape(num * num)
            dst_index = torch.arange(num, device=x.device).unsqueeze(0).expand(num, num).reshape(num * num)
            edge_index = torch.stack([src_index, dst_index])

            # Apply the scoring function (* represents element-wise (a.k.a. Hadamard) product)
            # shape = (N, NH, FOUT) * (1, NH, FOUT) -> (N, NH, 1) -> (N, NH) because sum squeezes the last dimension
            # Optimization note: torch.sum() is as performant as .sum() in my experiments
            scores_source = (nodes_features_proj[i] * self.scoring_fn_source).sum(dim=-1)
            scores_target = (nodes_features_proj[i] * self.scoring_fn_target).sum(dim=-1)
            ...
            nodes_features_proj_lifted_weighted = nodes_features_proj_lifted[i] * attentions_per_edge
            ...
            out_nodes_features = self.skip_concat_bias(attentions_per_edge, x[i], out_nodes_features)
            out_features.append(out_nodes_features)

        return torch.stack(out_features), boxes_num
